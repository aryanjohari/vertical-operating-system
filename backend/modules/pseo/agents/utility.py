...
def get_local_blurb(city: str, distance: str, anchor: str) -> str:
    ...
    Uses SAB (Service Area Business) friendly phrasing.
    """
    city = (city or "").strip()
    ...
    if not city:
        return ""
    if not anchor:
        return f"We proudly serve clients throughout the {city} area."
    
    return f"We proudly serve clients throughout {city}, providing dedicated support for those in or around {anchor}."
